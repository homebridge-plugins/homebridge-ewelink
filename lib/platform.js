...
export default class {
  ...
  applyUserConfig(config) {
    ...
    const logDecrease = (k, max) => {
      this.log.warn('%s [%s] %s %s.', this.lang.cfgItem, k, this.lang.cfgHigh, max)
    }
    const logQuotes = (k) => {
                ...
                switch (k) {
                  ...
                  case 'sensorTimeLength': {
                    ...
                    if (Number.isNaN(intVal)) {
                      ...
                      logDecrease(`${key}.${id}.${k}`, platformConsts.maxValues[k])
                      this.rfSubdevices[id][k] = platformConsts.maxValues[k]
                    } else {
              ...
              entries.forEach((subEntry) => {
                ...
                switch (k) {
                  ...
                  case 'overrideLogging':
                    // Every device file reads this, but it had no case here, so it
                    // was warned about as a setting to remove and never took effect
                    if (!['standard', 'debug', 'disable'].includes(v)) {
                      logIgnore(`${key}.${id}.${k}`)
                    } else {
                      this.deviceConf[id][k] = v
                    }
  ...
  async pluginSetup() {
    ...
    try {
      ...
      this.log.debugWarn = (...args) => this.log.debug(...args)
      ...
      if (this.config.apiPort !== 0 && this.config.password) {
        ...
        // A failure to listen - the chosen port already taken by another instance
        // or another plugin's api - arrives as an 'error' event, not through the
        // callback below. Node throws an 'error' event nobody is listening for,
        // so without this the child bridge died on a port clash and restarted
        // straight back into it, leaving every device showing No Response.
        this.apiServer.on('error', (err) => {
          this.log.warn('%s [%s].', this.lang.apiListenErr, parseError(err))
        })

        // Start listening on the above created server. Node calls this back only
        // on success, and with no arguments.
        this.apiServer.listen(this.config.apiPort === 1 ? 0 : this.config.apiPort, () => {
  ...
  applyAccessoryLogging(accessory) {
    // Debug lines always go to Homebridge's debug logger, which prints them only
    // when debug is on - so there is nothing to decide here. Working it out from
    // the process arguments instead is what lost debug logging entirely for
    // anyone running the plugin in a child bridge.
    accessory.logDebug = msg => this.log.debug('[%s] %s.', accessory.displayName, msg)
  ...
  async initialiseDevice(device) {
    try {
      ...
      if (
        ...
        || (platformConsts.devices.dualr3.includes(uiid)
      ...
      if (platformConsts.devices.dualr3.includes(uiid) && device.params.workMode === 2) {
        /**
         ***********************
        ...
        /**/
      } else if (platformConsts.devices.fanGear.includes(uiid)) {
        /**
         FANS [GEAR]
         *
         */
        accessory = devicesInHB.get(uuid) || this.addAccessory(device, `${device.deviceid}SWX`)
        this.applyAccessoryLogging(accessory)
        accessory.control = new deviceTypes.deviceFanGear(this, accessory)
        /**/
        ...
        for (let i = 0; i <= platformConsts.supportedDevices[uiid]; i += 1) {
          ...
          this.saveAccessoryUpdate(subAccessory)
          if (i === 0) {
        ...
        /** */
      } else if (platformConsts.devices.zbSiren.includes(uiid)) {
        /**
         **************
         ZB SIREN
         ***************
        ...
        accessory = devicesInHB.get(uuid) || this.addAccessory(device, `${device.deviceid}SWX`)
        this.applyAccessoryLogging(accessory)
        accessory.control = new deviceTypes.zb.deviceSiren(this, accessory)
        /** */
      ...
      accessory.context.showAs = device.showAs
      ...
      if (type === 'rf_sub') {
        ...
        if (['1', '2', '3', '4'].includes(switchNumber)) {
          ...
          if (device?.tags?.ck_channel_name?.[Number.parseInt(switchNumber, 10) - 1]) {
            ...
            newDeviceName = device.tags.ck_channel_name[Number.parseInt(switchNumber, 10) - 1]
          } else {
  ...
  saveAccessoryUpdate(accessory) {
    ...
    // accessory in our in-memory map. Hidden accessories are deliberately never
    // registered (see addAccessory), so they have no associated plugin and would
    // make Homebridge throw when it serialises its accessory cache to disk
    // ('Cannot serialize accessory ... - missing associated plugin'). The same
    // applies to external accessories (the tv/box/stick simulations), which are
    // published outside the bridge so have no associated platform. We still
    // keep them in devicesInHB so lookups (e.g. the SW0 master of a multi-channel
    // device) continue to work.
    if (!accessory.context.hidden && !accessory.context.isExternal) {
  ...
  async sendDeviceUpdate(accessory, params) {
    ...
    // Try LAN first (bypass queue for instant local response)
    let lanResult
    if (this.lanClient && platformConsts.devices.lan.includes(accessory.context.eweUIID)) {
      lanResult = await this.lanClient.sendUpdate(payload)
      if (lanResult === 'ok') {
        return true
      }
    }

    // Fall back to cloud via queue
    return queue.add(async () => {
      if (this.wsClient) {
        const reason = lanResult || (this.lanClient ? this.lang.lanNotSup : this.lang.lanDisabled)
        accessory.logDebug(`${this.lang.revertWS} ${reason}`)
        if (accessory.context.reachableWAN) {
          return this.wsClient.sendUpdate(payload)
        }
        throw new Error(this.lang.unreachable)
      } else {
        const eText = lanResult ? `${this.lang.unreachable} [${lanResult}]` : this.lang.lanDisabled
  ...
  async sendGroupUpdate(accessory, params) {
    ...
    return queue.add(async () => {
      ...
      await this.httpClient.updateGroup(accessory.context.eweDeviceId, params)
    })
  }
  ...
  async receiveDeviceUpdate(device) {
    ...
    if (reachableChange && accessory.context.hbDeviceId.slice(-1) !== 'X') {
