...
export default class {
  ...
  async externalUpdate(params) {
    try {
      if (params.switches?.length >= 4 && !this.inUse) {
        // One door at a time. This was a `forEach(async ...)`, so both doors ran
        // at once over a shared `this.inUse` flag - door one cleared it two
        // seconds in while door two was still moving, defeating the guard - and a
        // rejection from the awaits below escaped the try above rather than being
        // caught by it, because the callback's promise was thrown away.
        for (const v of ['1', '2']) {
