  ...
  async externalUpdate(params) {
    try {
      ...
      if (
        ...
        // RF Sensor
        // One channel at a time. This was a `forEach(async ...)`, so every
        // triggered channel ran at once and the promise each returned was thrown
        // away - the `sleep` calls below held nothing back, and a rejection
        // escaped the try in externalUpdate rather than being caught by it.
        for (const chan of Object.keys(params).filter(name => rfTrigRegex.test(name))) {
          try {
            this.devicesInHB.forEach((acc) => {
              if (
                acc.context.eweDeviceId === this.accessory.context.eweDeviceId
