...
export default class {
  constructor(platform, accessory) {
    ...
    // the same colour made paler or richer - reached nothing and the light
    // never changed. Both handlers share one update, and the debounce inside
    // it collapses a hue-and-saturation pair into a single command.
    this.service
      .getCharacteristic(this.hapChar.Saturation)
      .onSet(async () => this.internalColourUpdate(this.service.getCharacteristic(this.hapChar.Hue).value))
    ...
    if (this.alShift !== -1) {
      ...
      // Without this the cleanup that removes the adaptive lighting
      // characteristics can never run, because it is guarded on this flag - so
      // switching adaptive lighting off left Home still offering it.
  ...
  async internalColourUpdate(value) {
    try {
      // Saturation is part of "has the colour changed": comparing hue alone
      // meant a change of saturation at the same hue looked like no change at
      // all, and nothing was sent to the light.
      const currentSat = this.service.getCharacteristic(this.hapChar.Saturation).value
      if (this.cacheHue === value && this.cacheSat === currentSat) {
        return
      }
      const updateKey = generateRandomString(5)
      ...
      await this.platform.sendDeviceUpdate(this.accessory, params)
      this.cacheHue = value
      // Remember the saturation that was sent, or the check at the top of this
      // method compares against a value that never moves and a second change of
      // saturation looks like no change at all
      this.cacheSat = sat;
        ...
        case 173:
          ...
          if (hasProperty(params, 'colorR')) {
            ...
            if (
              ...
              || params.colorG !== this.cacheG
              || params.colorB !== this.cacheB
            ) {
              // A light repeating our own command back can land a unit or two
              // off. Beyond that, somebody set this colour themselves, outside
              // HomeKit - which nothing else notices, so adaptive lighting has
              // to be stopped here or it moves the light off that colour within
              // minutes. This used to add the three channels together and want
              // a total of 50 out of a possible 765, so a deliberate but subtle
              // choice was read as noise and ignored.
              const rgbDiff = Math.max(
                Math.abs(params.colorR - this.cacheR),
                Math.abs(params.colorG - this.cacheG),
                Math.abs(params.colorB - this.cacheB),
              )
