...
export default class {
  constructor(platform, accessory) {
    ...
    // SNZB-03P models also report a binary bright/dark light level (uiid 7002)
    if (accessory.context.eweUIID === 7002) {
  ...
  async externalUpdate(params) {
    try {
      ...
      if (
        ...
        const timeDiff = (Date.now() - params.trigTime) / 1000
  ...
  markStatus(isOnline) {
    ...
    // Reset stuck motion state when bridge goes offline
    if (!isOnline && this.cacheState === 1) {
      this.cacheState = 0
