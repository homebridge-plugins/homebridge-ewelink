...
export default class {
  constructor(platform, accessory) {
    ...
    // The Sonoff SC also reports dust, light and noise levels (uiid 18)
    if (this.accessory.context.eweUIID === 18) {
  ...
  async externalUpdate(params) {
    try {
      if (this.airService && hasProperty(params, 'dusty')) {
        // The device reports a level from 1 (best) to 10 (worst)
        const rawDust = Number.parseInt(params.dusty, 10)
        if (!Number.isNaN(rawDust)) {
          const newQuality = Math.max(Math.min(Math.ceil(rawDust / 2), 5), 1)
          if (newQuality !== this.cacheDust) {
            this.cacheDust = newQuality
            this.airService.updateCharacteristic(this.hapChar.AirQuality, this.cacheDust)
            if (params.updateSource && this.enableLogging) {
              this.log('[%s] %s [%s].', this.name, this.lang.curAirQuality, rawDust)
            }
          }
        }
      }
      if (this.lightService && hasProperty(params, 'light')) {
        // The device reports a level from 1 (darkest) to 10 (brightest),
        // mapped to a nominal lux value for HomeKit
        const rawLight = Number.parseInt(params.light, 10)
        if (!Number.isNaN(rawLight)) {
          const newLux = Math.max(rawLight * 10, 0.0001)
          if (newLux !== this.cacheLux) {
            ...
            if (params.updateSource && this.enableLogging) {
              this.log('[%s] %s [%s].', this.name, this.lang.curLightLevel, rawLight)
            }
          }
        }
      }
      if (hasProperty(params, 'temperature')) {
        const rawTemp = Number(params.temperature)
        if (Number.isNaN(rawTemp) || rawTemp < -270 || rawTemp > 270) {
          return
        }
        let newTemp = rawTemp
      ...
      if (hasProperty(params, 'humidity')) {
        const rawHumi = Number.parseInt(params.humidity, 10)
        if (rawHumi === 0) {
          return
        }
        let newHumi = rawHumi
  ...
  async updateCache() {
    ...
    // Update in-memory temperature cache for cross-device sharing
    if (this.platform.temperatureCache) {
      this.platform.temperatureCache.set(this.accessory.context.eweDeviceId, this.cacheTemp)
    }

    // Attempt to save the new temperature to the persistent cache
    try {
