...
export default class {
  ...
  async getHost(attempt = 0) {
    ...
    try {
      ...
      this.log.debug('%s [%s].', this.lang.wsHostRec, body.domain)
  ...
  async login() {
    ...
    try {
      ...
      this.wsClient.onOpen.addListener(async () => {
        ...
        this.log.debug('%s.', this.lang.wsLogin)
          ...
          if (res.config && res.config.hb && res.config.hbInterval) {
            ...
            this.hbInterval = setInterval(() => {
              try {
                ...
                this.log.debugWarn('%s %s.', this.lang.wsPingError, parseError(err))
              }
            }, (res.config.hbInterval + 7) * 1000)

            // Login was successful and log if appropriate
            this.authRetries = 0
            this.log.debug('%s.', this.lang.wsLoginSuccess)
      ...
      this.wsClient.onClose.addListener(async (e) => {
        ...
        this.log.debugWarn('%s [%s].', this.lang.wsReconnectError, e)
      ...
      this.wsClient.onError.addListener(async (e) => {
        this.log.debugWarn('%s [%s].', this.lang.wsReconnectClose, e)
      ...
      this.log.debug('%s: %s.', this.lang.wsLoginErrRecon, errToShow)
  ...
  async closeConnection() {
    ...
    if (this.wsClient) {
      ...
      if (this.wsIsConnected) {
        ...
        // Log if appropriate
        this.log.debug('%s.', this.lang.stoppedWS)
      }
    }
  }
}
