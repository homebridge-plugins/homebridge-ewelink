...
export default class {
  constructor(platform) {
    ...
    // A stalled connection would otherwise hang requests forever with no error,
    // leaving the plugin stuck on 'fetching devices' at startup (#788).
    // A timeout surfaces as ECONNABORTED/ETIMEDOUT, which the retry logic handles.
  ...
  async login() {
    try {
      ...
      this.log.debug('%s.', this.lang.sendLogin)
      ...
      if (body.error === 10004 && body?.data?.region) {
        ...
        this.log.debug('%s [%s].', this.lang.newRegionRec, this.httpHost)
    ...
    try {
      ...
      fullDeviceList.forEach((d) => {
        ...
        if (d?.itemData?.extra?.uiid && !this.ignoredDevices.includes(d.itemData.deviceid)) {
          ...
          // sensorSwitches is a list of device ids, so it has to be searched, not
          // indexed. Indexing it always gave undefined, so a non-DW2 sensor was
          // never moved to the end of the list and could be set up before the
          // garage that needs it - which fell back to a plain contact sensor until
          // the next restart.
          const isSensorSwitch = this.sensorSwitches.includes(d.itemData.deviceid)
          if (
