export default {
  ...
  // it does not throw - it quietly becomes 1 ms, so a device set past that point
  // would run its reset timer immediately and repeatedly. operationTime is
  // multiplied by 100 and sensorTimeLength by 1000, so both are capped well
  // inside the limit.
  maxValues: {
    operationTime: 2073600,
    operationTimeDown: 2073600,
    sensorTimeDifference: 2073600,
    sensorTimeLength: 2073600,
  },
  ...
  devices: {
    ...
    switchSingle: [1, 6, 14, 24, 27, 216, 225, 226, 268, 1009, 1256, 7004, 7005, 7010, 7028, 20001],
    switchSCM: [77, 78, 81, 107, 112, 138, 160, 191, 209, 264],
    switchSCMPower: [182, 190, 276, 7032],
    ...
    switchMultiPower: [126, 130, 262],

    // The DUALR3 family can be switched into motor (workMode 2) or meter
    // (workMode 3) mode. This is kept separate from switchMultiPower because
    // the Lite has those modes but no power monitoring (#778).
    dualr3: [126, 165],
    switchMate: [177],
    switchMan: [174],
    lightDimmer: [36, 44, 57, 277, 20006],
    lightRGB: [22],
    lightCCT: [52, 103],
    ...
    zbSwitchStateless: [1000, 7000, 7039],
    zbLightDimmer: [1257],
    zbLightCCT: [1258],
    zbLightRGBCCT: [3258, 7009],
    zbMotor: [1514, 7006, 7034],
    zbSensorAmbient: [242, 1770, 1771, 7014, 7033, 7038],
    zbSensorMotion: [2026, 7002],
    zbSensorOccupancy: [7016, 7055],
    zbSensorContact: [3026, 7003],
    zbSensorWater: [4026, 7019],
    zbSensorSmoke: [5026],
    zbSiren: [7056],
    zbThermostat: [7017],
    zbWaterValve: [7027, 7047],
    group: [5000],
  ...
  supportedDevices: {
    ...
    130: 4, // SPM-4Relay sub unit
    242: 1, // Wireless temperature and humidity sensor
    133: 1, // "NSPANEL"
    135: 1, // ""
    ...
    7038: 1, // SONOFF AirGuard TH Zigbee temperature and humidity sensor (#767)
    7040: 2, // SONOFF MINI DUO 2-gang Zigbee switch (#768)
    7047: 1, // SONOFF Zigbee Irrigation Valve (#778)
    7055: 1, // SNZB-03PR2 Zigbee presence sensor with light level
    7056: 1, // SNZB-09P Zigbee indoor siren
    20001: 1, // iHost sub-device single switch
    20004: 4, // iHost sub-device 4-channel switch
    20006: 1, // single-colour dimmable LED controller (#780)
  },
